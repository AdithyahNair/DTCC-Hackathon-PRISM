      ...
      const fetchFairnessData = async () => {
        ...
        try {
          ...
          if (response.data.metrics) {
            Object.entries(response.data.metrics).forEach(
              ([feature, metrics]: [string, any]) => {
                console.log(`\n=== Metrics for ${feature} ===`);
                ...
                if (metrics.interpretation) {
                  ...
                  console.log(
                    ...
                    metrics.interpretation.demographic_parity_threshold
                  );
                  console.log(
                    "Equal Opportunity Threshold:",
                    metrics.interpretation.equal_opportunity_threshold
                  );
                  console.log(
                    "Disparate Impact Threshold:",
                    metrics.interpretation.disparate_impact_threshold
                  );
                  console.log(
                    "Statistical Parity Threshold:",
                    metrics.interpretation.statistical_parity_threshold
                  );
                }
    ...
    >
      ...
      {fairnessAPIData && (
        <>
          ...
          <div className="bg-white rounded-xl p-6 shadow-md border border-gray-100">
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {fairnessAPIData.metrics.sensitive_features?.map(
          ...
          {fairnessAPIData.metrics.metrics &&
            Object.entries(fairnessAPIData.metrics.metrics).map(
              ([feature, metrics]: [string, any]) => {
                const demographicParityData = processMetricDataForChart(
                  metrics.demographic_parity,
                  "Demographic Parity"
                );
                const equalOpportunityData = processMetricDataForChart(
                  metrics.equal_opportunity,
                  "Equal Opportunity"
                );
                const statisticalParityData = processMetricDataForChart(
                  metrics.statistical_parity,
                  "Statistical Parity"
                );

                // Combine data for multi-line chart
                const combinedData = demographicParityData.map(
                  (item, index) => ({
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                      <div className="p-4 border border-gray-200 rounded-lg">
                        ...
                        <p className="text-lg font-bold text-gray-900">
                          {dpSummary.avg.toFixed(4)}
                        </p>
                        <p className="text-xs text-gray-500">
                          Avg across {dpSummary.count} thresholds
                        </p>
                      </div>
                      ...
                      <div className="p-4 border border-gray-200 rounded-lg">
                        ...
                        <p className="text-lg font-bold text-gray-900">
                          {eoSummary.avg.toFixed(4)}
                        </p>
                        <p className="text-xs text-gray-500">
                          Avg across {eoSummary.count} thresholds
                        </p>
                      </div>
                      ...
                      <div className="p-4 border border-gray-200 rounded-lg">
                        ...
                        <p className="text-lg font-bold text-gray-900">
                          {diSummary.count > 0
                            ? diSummary.avg.toFixed(4)
                            : "No data"}
                        </p>
                        <p className="text-xs text-gray-500">
                          {diSummary.count > 0
                            ? `Avg across ${diSummary.count} thresholds`
                            : "No threshold data"}
                        </p>
                      </div>
                      ...
                      <div className="p-4 border border-gray-200 rounded-lg">
                        ...
                        <p className="text-lg font-bold text-gray-900">
                          {spSummary.avg.toFixed(4)}
                        </p>
                        <p className="text-xs text-gray-500">
                          Avg across {spSummary.count} thresholds
                        </p>
                      </div>
                    </div>
                    ...
                    {combinedData.length > 0 && (
                      <div className="mb-6">
                        ...
                        <p className="text-sm text-gray-600 mt-2">
                          This chart shows how fairness metrics vary across
                          different threshold values. Values closer to 0
                          generally indicate better fairness.
                        </p>
                      </div>
                    )}
                            ...
                            data={[
                              {
                                ...
                                average: dpSummary.avg,
                                min: dpSummary.min,
                                max: dpSummary.max,
                              },
                              {
                                metric: "Equal Opportunity",
                                average: eoSummary.avg,
                                min: eoSummary.min,
                                max: eoSummary.max,
                              },
                              {
                                metric: "Statistical Parity",
                                average: spSummary.avg,
                                min: spSummary.min,
                                max: spSummary.max,
                              },
                            ]}
                    ...
                    {metrics.interpretation && (
                      <div className="p-4 bg-gray-50 rounded-lg">
                        ...
                        <p className="text-xs text-gray-500 mt-2">
                          Values within these thresholds are considered
                          acceptable for fairness.
                        </p>
                      </div>
                    )}
                  </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-md border border-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <span className="ml-2 font-medium">
                  {fairnessAPIData.model_name} v{fairnessAPIData.model_version}
                </span>
              </div>
              <div>
