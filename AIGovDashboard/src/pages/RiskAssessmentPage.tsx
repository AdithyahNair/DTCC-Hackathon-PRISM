...
const RiskAssessmentPage: React.FC = () => {
  ...
  const [expandedSections, setExpandedSections] = useState<Set<number>>(
    new Set()
  ); // Start with no sections expanded
    ...
    try {
      ...
      try {
        const response = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=AIzaSyD6L9OZMjl5CMuwcezKH6dwZPF4oB5EKv8`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
...
Section 10: AI Accountability
Description: This section ensures the organization has risk management mechanisms to effectively manage identified AI risk.
Status: Analysis completed - Standard controls implemented
Recommendation: Implement comprehensive risk management framework with regular audits and independent third-party assessments
    ...
    try {
      ...
      const allSections = [
        ...
        // Section 3: Valid and Reliable AI (Auto section)
        {
          ...
          recommendation:
            "Maintain regular impact assessments and ensure compliance with all applicable regulations. Implement continuous monitoring for societal and environmental impacts.",
        },
        ...
        // Section 5: Secure and Resilient AI (Auto section)
        {
          ...
          recommendation:
            "Implement comprehensive security testing including red-team exercises and continuous vulnerability assessments. Establish robust incident response procedures.",
        },
        // Section 6: Explainable and Interpretable AI (Auto section)
        {
          ...
          recommendation:
            "Enhance traceability mechanisms and implement comprehensive logging. Ensure all decisions can be explained to relevant stakeholders with appropriate detail.",
        },
        ...
        // Section 8: Fairness and Unbiased AI (Auto section)
        {
          ...
          recommendation:
            "Implement comprehensive bias testing and monitoring. Ensure diverse representation in development teams and training data to promote fairness and inclusivity.",
        },
        // Section 9: Transparent and Accountable AI (Auto section)
        {
          ...
          recommendation:
            "Establish clear communication protocols for all stakeholders. Ensure users are properly informed about AI interactions and system limitations.",
        },
        // Section 10: AI Accountability (Auto section)
        {
          ...
          recommendation:
            "Implement comprehensive risk management framework with regular audits. Establish procedures for independent third-party assessments and continuous trustworthiness verification.",
        },
      ];
        ...
        if (section.isUser) {
          ...
          if (sectionAnswers.length > 0) {
            ...
            if (section.title === "AI System Information") {
              const hasMLSystem = sectionAnswers.some(
                (sa: any) =>
              ...
              const hasCloudDeployment = sectionAnswers.some(
                (sa: any) =>
                  sa.answer.toLowerCase().includes("cloud") ||
                  sa.answer.toLowerCase().includes("saas")
              );
              const hasAutomatedDecisions = sectionAnswers.some(
                (sa: any) =>
                  sa.answer.toLowerCase().includes("decision") ||
                  sa.answer.toLowerCase().includes("automat")
              );

              overallRecommendation = `Based on your AI system profile, we recommend: ${
                hasMLSystem
              ...
              const hasDocumentedRoles = sectionAnswers.some(
                (sa: any) =>
                  sa.answer.toLowerCase().includes("yes") &&
                  sa.question.field === "rolesDocumented"
              );
              const hasTraining = sectionAnswers.some(
                (sa: any) =>
                  sa.answer.toLowerCase().includes("yes") &&
                  (sa.question.field === "personnelTrained" ||
                    sa.question.field === "biasTraining")
              );
              const hasHumanOversight = sectionAnswers.some(
                (sa: any) =>
                  sa.answer.toLowerCase().includes("human") ||
                  sa.answer.toLowerCase().includes("oversight")
              );

              overallRecommendation = `Your human governance approach shows ${
                hasDocumentedRoles
              ...
              const hasHighRisk = sectionAnswers.some(
                (sa: any) =>
                  sa.answer.toLowerCase().includes("high") ||
                  sa.answer.toLowerCase().includes("critical")
              );
              const hasThreatAssessment = sectionAnswers.some(
                (sa: any) =>
                  sa.answer.toLowerCase().includes("yes") &&
                  sa.question.field === "threatsIdentified"
              );
              const hasMaliciousUseAssessment = sectionAnswers.some(
                (sa: any) =>
                  sa.answer.toLowerCase().includes("yes") &&
                  sa.question.field === "maliciousUseAssessed"
              );

              overallRecommendation = `Your safety and reliability profile indicates ${
                hasHighRisk
              ...
              const usesPersonalData = sectionAnswers.some(
                (sa: any) =>
                  sa.answer.toLowerCase().includes("yes") &&
                  sa.question.field === "personalInfoUsed"
              );
              const hasRegulationCompliance = sectionAnswers.some(
                (sa: any) =>
                  sa.answer.toLowerCase().includes("gdpr") ||
                  sa.answer.toLowerCase().includes("ccpa") ||
                  (sa.answer.toLowerCase().includes("yes") &&
                    sa.question.field === "privacyRegulations")
              );
              const hasPrivacyByDesign = sectionAnswers.some(
                (sa: any) =>
                  sa.answer.toLowerCase().includes("yes") &&
                  sa.question.field === "privacyByDesign"
              );
              const hasDataQuality = sectionAnswers.some(
                (sa: any) =>
                  sa.answer.toLowerCase().includes("automated") ||
                  sa.answer.toLowerCase().includes("continuous") ||
                  (sa.answer.toLowerCase().includes("yes") &&
                    sa.question.field === "dataQuality")
              );

              overallRecommendation = `Your privacy and data governance approach ${
                usesPersonalData
      ...
      localStorage.setItem(
        `riskAssessment_${id}`,
        JSON.stringify(updatedAnalysisData)
      );

      // Also store in reports list for easy access
      const existingReports = JSON.parse(
        localStorage.getItem("riskAssessmentReports") || "[]"
      );
      const updatedReports = existingReports.filter(
        (report: any) => report.projectId !== id
      );
      updatedReports.push({
        projectId: id,
        projectName,
      ...
      alert(
        "✅ AI Risk Assessment analysis completed and saved to storage! You can download the report from the Reports page."
      );
    } catch (error) {
          ...
          >
            <span
              className={`font-semibold ${
                isAutoCompleted ? "text-green-600" : "text-teal-600"
              }`}
            >
              {sectionNumber}
            </span>
